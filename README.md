...
- **🎚 Nivel de esfuerzo** — cuánto razona antes de responder (`bajo` … `máximo`), con herencia perfil→proyecto y **también por agente**, como el modelo. Por defecto usa el del CLI, así que no cambia nada si no lo tocas.
- **📢 Broadcast** — `@todos <mensaje>` envía el mismo prompt a todos los agentes libres a la vez.
- **`/repartir`** — fuerza el reparto sin esperar a que el agente lo decida, y al terminar la respuesta dice cuánto contexto se trabajó **fuera** de este hilo.
